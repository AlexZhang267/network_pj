...
import string
import time
import http_part.server
from threading import Thread

try:
    from http_part.server import HTTPStatus
except ImportError:
    class HTTPStatus:
        OK = 200
        NOT_FOUND = 404
...
class TestHTTPServer(http_part.server.HTTPServer):
    def __init__(self, server_port, request_handler_class):
        self.allow_reuse_address = True
        super().__init__(('0.0.0.0', server_port), request_handler_class)
...
class DifferentEncodingRspHandler(http_part.server.BaseHTTPRequestHandler):
