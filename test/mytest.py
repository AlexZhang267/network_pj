...
def test_get(command, url, extra_func=None, timeout=16, query=""):
    stdout_file = open('test_get_out.txt', 'w')
    stderr_file = open('test_get_err.txt', 'w')
    if query:
        url = url + '?' + query
...
def test_resolve(self, host, family=socket.AF_INET):
    ...
    _, stderr, _ = test_get(test_cmd, "http_part://" + host, extra_func=func, timeout=6)
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_dns_resolve_not_exist(self):
        ...
        _, _, exit_code = test_get(test_cmd, "http_part://" + host, timeout=6)
    ...
    def test_basic_http(self):
        _, test = test_fetch_content(self, "http_part://www.fudan.edu.cn/2016/index.html")
        ...
        _, test = test_fetch_content(self, "http_part://www.xiami.com", simple_check=True)
    ...
    def test_http_with_port(self):
        test_fetch_content(self, "http_part://www.urp.fudan.edu.cn:92/eams/login.action",
                           simple_check=True)

    @unittest.skip("Invalid test")
    def test_url_parsing_sp(self):
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8033/test_url_parsing",
    ...
    def test_broken_response(self):
        ...
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/0")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/1")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/2")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/3")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/4")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/5")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/6")
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/7")
        # Chunked transfer encoding
        test_exit_0("http_part://pj-test.htcnet.moe:8031/test/8")

    def test_delayed_response(self):
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8032/test/10")
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8032/test/11")
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8032/test/12")
    ...
    @unittest.skipIf(not support_ipv6, "IPV6 is not supported")
    def test_ipv6(self):
        test_fetch_content(self, "http_part://ftp6.sjtu.edu.cn")
        test_fetch_content(self, "http_part://ftp6.sjtu.edu.cn/centos/RPM-GPG-KEY-CentOS-7")
    ...
    def test_ipv4_ipv6(self):
        ...
        test_fetch_content(self, "http_part://pj-test-dns-v4-v6-invalid.htcnet.moe/test")

    def test_chunked_coding(self):
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8033/test/20")

    def test_gzip_coding(self):
        _, test = test_fetch_content(self, "http_part://pj-test.htcnet.moe:8033/test/21")
        self.assertTrue(test["stderr"].find(b"Content-Encoding: gzip") >= 0,
                        "Gzip coding not supported.")
        test_fetch_content(self, "http_part://pj-test.htcnet.moe:8033/test/22")
